...
class SecApiClient:
    ...
    @retry(wait=wait_exponential(multiplier=2, min=2, max=60), stop=stop_after_attempt(5), retry=retry_if_exception(_is_rate_limit_error), reraise=True)
    ...
    def get_latest_filings(self, ticker: str) -> dict[str, dict]:
        filings = {}
        annual_query = {"query": {"query_string": {"query": f'ticker:"{ticker}" AND (formType:"10-K" OR formType:"20-F" OR formType:"40-F" OR formType:"10-KT")'}}, "from": "0", "size": "1", "sort": [{"filedAt": {"order": "desc"}}]}
        filings["annual"] = self._get_latest_filing(annual_query)
        quarterly_query = {"query": {"query_string": {"query": f'ticker:"{ticker}" AND (formType:"10-Q" OR formType:"10-QT")'}}, "from": "0", "size": "1", "sort": [{"filedAt": {"order": "desc"}}]}
        filings["quarterly"] = self._get_latest_filing(quarterly_query)
        return filings

    @retry(wait=wait_exponential(multiplier=2, min=2, max=60), stop=stop_after_attempt(3), retry=retry_if_exception(_is_rate_limit_error), reraise=True)
