...
def _delete_old_dashboard_files(ticker: str):
    ...
    for blob_name in blobs_to_delete:
        try:
            ...
            logging.error(
                f"[{ticker}] Failed to delete old dashboard file {blob_name}: {e}"
            )
...
def _get_price_chart_data(ticker: str) -> Optional[Dict[str, Any]]:
    ...
    latest_blob = gcs.get_latest_blob_for_ticker(
        config.GCS_BUCKET_NAME, PRICE_CHART_JSON_FOLDER, ticker
    )
    if latest_blob:
        try:
...
def process_prep_file(prep_blob_name: str) -> Optional[str]:
    ...
    if not match:
        logging.warning(
            f"Could not parse ticker/date from prep file name: {prep_blob_name}. Skipping."
        )
        return None
    ...
    try:
        ...
        gcs.write_text(
            ...
            output_blob_name,
            json.dumps(final_dashboard, indent=2),
        )
        ...
        logging.error(
            f"[{ticker}] CRITICAL ERROR during dashboard generation from {prep_blob_name}: {e}",
            exc_info=True,
        )
        return None
...
def run_pipeline():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_item = {
            executor.submit(process_prep_file, item): item for item in work_items
        }
        for future in as_completed(future_to_item):
            try:
                ...
                logging.error(
                    f"Prep file {future_to_item[future]} caused an unhandled exception: {exc}",
                    exc_info=True,
                )

    logging.info(
        f"--- Dashboard Generation Pipeline Finished. Successfully generated {processed_count} of {len(work_items)} dashboards. ---"
    )
