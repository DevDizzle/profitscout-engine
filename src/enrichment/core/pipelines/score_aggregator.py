...
def _read_and_parse_blob(
    blob_name: str, analysis_type: str, storage_client: storage.Client
) -> tuple | None:
    """
    ...
    try:
        ...
        if analysis_type == "business_summary":
            ...
            if analysis_match:
                parsed_data[f"{analysis_type}_analysis"] = (
                    analysis_match.group(1).replace("\\n", " ").strip()
                )
    ...
    with ThreadPoolExecutor(max_workers=config.MAX_WORKERS * 4) as executor:
        ...
        for analysis_type, prefix in all_prefixes.items():
            ...
            blobs = gcs.list_blobs(
                config.GCS_BUCKET_NAME, prefix, client=storage_client
            )

            for blob_name in blobs:
                # --- FIX: Pass shared client to worker ---
                future = executor.submit(
                    _read_and_parse_blob, blob_name, analysis_type, storage_client
                )
        ...
        for future in as_completed(future_to_blob):
            ...
            if processed_count % 500 == 0:
                logging.info(
                    f"    ..... Progress: {processed_count}/{total_futures} files..."
                )
...
def _calculate_regime_weighted_score(row: pd.Series) -> float:
    ...
    weights = (
        config.SCORE_WEIGHTS_EVENT if is_event_regime else config.SCORE_WEIGHTS_QUIET
    )
...
def _process_and_score_data(ticker_data: dict) -> pd.DataFrame:
    ...
    df = (
        pd.DataFrame.from_dict(ticker_data, orient="index")
        .reset_index()
...
def run_pipeline():
    ...
    if final_df.empty:
        ...
        return
    logging.info(
        f"STEP 2 COMPLETE: Processed data into a DataFrame with shape {final_df.shape}."
    )

    logging.info("STEP 3: Starting to load DataFrame to BigQuery...")
    job_config = bigquery.LoadJobConfig(write_disposition="WRITE_TRUNCATE")
    job = client.load_table_from_dataframe(
        final_df, config.SCORES_TABLE_ID, job_config=job_config
    )
    job.result()
    logging.info(
        f"STEP 3 COMPLETE: Loaded {job.output_rows} rows into BigQuery table: {config.SCORES_TABLE_ID}"
    )
