...
def _delete_old_price_json(ticker: str):
    ...
    for blob_name in blobs_to_delete:
        try:
            ...
            logging.error(
                f"[{ticker}] Failed to delete old price chart JSON {blob_name}: {e}"
            )
...
def _generate_price_chart_json(ticker: str, price_df: pd.DataFrame) -> str | None:
    ...
    df = df.dropna(
        subset=["date", "adj_close", "open", "high", "low", "volume"]
    ).sort_values("date")
    ...
    chart_data = {
        ...
        "sma50": [
            ...
            for row in plot_df.itertuples()
            if hasattr(row, "sma_50") and pd.notna(row.sma_50)
        ],
        "sma200": [
            {"date": row.date.strftime("%Y-%m-%d"), "value": row.sma_200}
            for row in plot_df.itertuples()
            if hasattr(row, "sma_200") and pd.notna(row.sma_200)
        ],
    }
    ...
    try:
        ...
        logging.info(
            f"[{ticker}] Successfully uploaded price chart JSON to gs://{config.GCS_BUCKET_NAME}/{blob_name}"
        )
        return blob_name
    except Exception as e:
        logging.error(
            f"[{ticker}] Failed to upload price chart JSON: {e}", exc_info=True
        )
        return None
...
def run_pipeline():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_ticker = {
            executor.submit(process_ticker, t, price_histories): t for t in tickers
        }
    ...
    logging.info(
        f"--- Price Chart JSON Generation Finished. Processed {processed_count} of {len(tickers)} tickers. ---"
    )
