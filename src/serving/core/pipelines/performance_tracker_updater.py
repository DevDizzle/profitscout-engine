...
def _get_current_prices(
    ...
    max_fetch_date_query = (
        f"SELECT MAX(fetch_date) as max_date FROM `{OPTIONS_CHAIN_TABLE_ID}`"
    )
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            bigquery.ArrayQueryParameter(
                "contract_symbols", "STRING", contract_symbols
            ),
            bigquery.ScalarQueryParameter(
                "latest_fetch_date", "DATE", latest_fetch_date
            ),
        ]
    )
...
def _upsert_with_merge(bq_client: bigquery.Client, df: pd.DataFrame):
    ...
    temp_table_id = (
        f"{TRACKER_TABLE_ID}_temp_staging_{pd.Timestamp.now().strftime('%Y%m%d%H%M%S')}"
    )
...
def run_pipeline():
    ...
    new_signals_df, active_contracts_df = _get_new_signals_and_active_contracts(
        bq_client
    )
    ...
    if not active_contracts_df.empty:
        active_contracts_df["expiration_date"] = pd.to_datetime(
            active_contracts_df["expiration_date"]
        ).dt.date
    ...
    if not active_contracts_df.empty:
        ...
        active_ongoing_df = active_contracts_df[
            active_contracts_df["expiration_date"] >= today
        ].copy()
        # If Exp Date < Today, it has expired.
        active_expired_df = active_contracts_df[
            active_contracts_df["expiration_date"] < today
        ].copy()
    ...
    if not new_signals_df.empty:
        ...
        new_signals_df["initial_price"] = (
            new_signals_df["signal_bid"] + new_signals_df["signal_ask"]
        ) / 2
        new_signals_df["initial_price"] = new_signals_df["initial_price"].fillna(
            new_signals_df["signal_last"]
        )
        ...
        new_signals_df = pd.merge(
            new_signals_df, current_prices_df, on="contract_symbol", how="left"
        )
        ...
        new_signals_df["current_price"] = new_signals_df["current_price"].fillna(
            new_signals_df["initial_price"]
        )
    ...
    if not active_ongoing_df.empty:
        ...
        active_ongoing_df = pd.merge(
            active_ongoing_df, current_prices_df, on="contract_symbol", how="left"
        )

        # Coerce numeric
        for col in ["initial_price", "current_price", "old_price"]:
            active_ongoing_df[col] = pd.to_numeric(
                active_ongoing_df[col], errors="coerce"
            )
        ...
        active_ongoing_df["current_price"] = active_ongoing_df["current_price"].fillna(
            active_ongoing_df["old_price"]
        )
        ...
        active_ongoing_df["current_price"] = (
            active_ongoing_df["current_price"]
            .fillna(active_ongoing_df["initial_price"])
            .fillna(0.0)
        )
        ...
        active_ongoing_df.loc[mask, "percent_gain"] = (
            (
                active_ongoing_df.loc[mask, "current_price"]
                - active_ongoing_df.loc[mask, "initial_price"]
            )
            / active_ongoing_df.loc[mask, "initial_price"]
            * 100
        ).round(2)
    ...
    if not active_expired_df.empty:
        ...
        active_expired_df["current_price"] = pd.to_numeric(
            active_expired_df["current_price"], errors="coerce"
        ).fillna(0.0)
        active_expired_df["percent_gain"] = pd.to_numeric(
            active_expired_df["percent_gain"], errors="coerce"
        ).fillna(-100.0)
