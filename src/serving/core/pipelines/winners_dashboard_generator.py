...
def run_pipeline():
    ...
    if strong_options_df.empty:
        logging.warning(
            "No strong options signals found. Winners table will be cleared."
        )
        bq.load_df_to_bq(
            empty_df,
            OUTPUT_TABLE_ID,
    ...
    if "outlook_signal" in winners_complete.columns:
        winners_complete["outlook_signal"] = winners_complete["outlook_signal"].fillna(
            "Neutral"
        )
    else:
    ...
    for _, row in final_df.iterrows():
        ...
        if hasattr(run_date_val, "strftime"):
            ...
            run_date_str = str(run_date_val).split(" ")[
                0
            ]  # Fallback for string "YYYY-MM-DD HH:MM:SS"
    ...
    logging.info("--- Winners Dashboard Generation Pipeline Finished ---")
