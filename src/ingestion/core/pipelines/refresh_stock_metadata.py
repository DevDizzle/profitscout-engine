...
def _get_existing_metadata_status(bq_client: bigquery.Client, tickers: list[str]) -> dict:
    """
    ...
    status = {t: None for t in tickers}
    try:
        ...
        logging.warning(f"Could not query BigQuery for existing metadata (might be empty/missing): {e}")
            ...
            try:
                ...
                if transcript_data and (earnings_call_date := transcript_data.get("date")):
                    records.append({
                        ...
                        "quarter_end_date": transcript_data.get("fillingDate"), # FMP often uses fillingDate or date roughly
...
def run_pipeline(fmp_client: FMPClient, bq_client: bigquery.Client, storage_client: storage.Client, publisher_client: pubsub_v1.PublisherClient):
    ...
    logging.info(f"Found {len(work_items)} stale tickers. Processing batch of {len(batch_tickers)}.")
    ...
    final_df['quarter_end_date'] = final_df['quarter_end_date'].fillna(final_df['earnings_call_date'])
    ...
    logging.info(f"Successfully merged {len(final_df)} records. Approx {remaining_items} stale tickers remaining.")
