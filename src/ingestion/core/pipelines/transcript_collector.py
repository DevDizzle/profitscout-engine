...
from .. import config
from ..gcs import upload_json_to_gcs, get_tickers, blob_exists
from ..clients.fmp_client import FMPClient

def _process_latest_transcript(ticker: str, fmp_client: FMPClient, storage_client: storage.Client):
...
def run_pipeline(fmp_client: FMPClient, bq_client: bigquery.Client, storage_client: storage.Client):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_process_latest_transcript, ticker, fmp_client, storage_client): ticker
