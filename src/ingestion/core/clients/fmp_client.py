...
class FMPClient:
    ...
    def get_financial_statements(self, ticker: str, limit: int) -> dict[str, list]:
        ...
        for key, statement_type in types.items():
            params = {"period": "quarter", "limit": limit}
            statements[key] = self._make_request(
                f"{statement_type}/{ticker}", params=params
            )
        return statements
    ...
    def fetch_prices_for_populator(
    ...
    def fetch_calendar(
        self, endpoint: str, start: datetime.date, end: datetime.date
    ) -> list[dict]:
