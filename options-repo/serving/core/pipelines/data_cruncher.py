...
def _calculate_technicals(price_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(df) >= 31:
        ...
        df['close_30d_delta_pct'] = (df['adj_close'] - df['close_30d_ago']) / df['close_30d_ago'] * 100
...
def _fetch_and_calculate_kpis(ticker: str, industry_map: Dict[str, float]) -> Optional[str]:
    ...
    try:
        ...
        if price_df.empty:
            logging.warning(f"[{ticker}] No price history found. Skipping prep file generation.")
