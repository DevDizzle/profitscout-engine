...
def _get_candidate_tickers(bq_client: bigquery.Client) -> list[str]:
    ...
    try:
        ...
        logging.error(f"Failed to load tickers from options_candidates: {e}", exc_info=True)
        return []
...
def _process_ticker(ticker: str, chain_df: pd.DataFrame, price_history_df: pd.DataFrame) -> Optional[dict]:
    """
    Worker function to process data for one ticker.
    """
    try:
        if price_history_df.empty:
            logging.warning(f"[{ticker}] No price history data found. Cannot generate features.")
            return None
        ...
        if not chain_df.empty:
            uprice = chain_df["underlying_price"].dropna().iloc[0] if "underlying_price" in chain_df.columns and not chain_df["underlying_price"].dropna().empty else latest_price_row['close']
            iv_avg = helper.compute_iv_avg_atm(chain_df, uprice, as_of_date)
            
            hv_30_for_signal = helper.compute_hv30(None, ticker, as_of_date, price_history_df=price_history_df)
            if iv_avg is not None and hv_30_for_signal is not None:
                iv_signal = "high" if iv_avg > (hv_30_for_signal + 0.10) else "low"
        
        hv_30 = helper.compute_hv30(None, ticker, as_of_date, price_history_df=price_history_df)
...
def _truncate_and_load_results(bq_client: bigquery.Client, df: pd.DataFrame):
    ...
    try:
        ...
        job.result()
        logging.info(f"Successfully truncated and loaded {job.output_rows} rows into {table_id}")
    except Exception as e:
...
def run_pipeline():
    ...
    logging.info("--- Starting Options Feature Engineering Pipeline (Wipe and Replace Mode) ---")
    ...
    if not tickers:
        ...
        bq_client.query(f"TRUNCATE TABLE `{config.PROJECT_ID}.{config.BIGQUERY_DATASET}.options_analysis_input`").result()
        return
    ...
    if not results:
        logging.warning("No features were generated after processing. Table will be empty.")
        bq_client.query(f"TRUNCATE TABLE `{config.PROJECT_ID}.{config.BIGQUERY_DATASET}.options_analysis_input`").result()
        return
