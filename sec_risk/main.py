...
GCS_FOLDER        = "sec-risk/"
COLD_FOLDER       = "sec-risk-cold/"
RETENTION_MONTHS  = 24                 # keep ≈ 8 quarters
PAUSE_SECONDS     = 0.25               # polite gap between tickers
...
def process_ticker(ticker: str, query_api: QueryApi, extractor_api: ExtractorApi):
    # 1) Query newest 10-K or 10-Q by reporting period
    query = {
        "query": {"query_string": {
    ...
    if bucket.blob(blob_path).exists():           # latest already stored
...
def produce(request):
    ...
    topic_path = publisher.topic_path(os.getenv("PROJECT_ID"), "sec-risk-sync")
...
def main(request):
    ...
    for ticker in get_tickers():
        ...
        time.sleep(PAUSE_SECONDS)     # keep QPS very low

    return "Risk Factors incremental fetch complete", 200
