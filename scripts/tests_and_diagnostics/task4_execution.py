...
def run_execution_test():
    print("="*80)
    print("TASK 4D: EXECUTION VIABILITY ON GATED COHORT")
    print("Target Cohort: SCORE_GTE_2 | Gate: Spread <= 15%")
    print("Execution Rule: 15:00 ET Entry | +40% Target | -25% Stop | 3-Day Max Hold")
    print("="*80)
    
    client = bigquery.Client(project=PROJECT_ID)
    query = """
    SELECT ticker, scan_date, recommended_contract, recommended_strike, recommended_expiration, direction, premium_score, recommended_spread_pct
    ...
    WHERE premium_score >= 2 
      ...
      AND recommended_spread_pct <= 0.15
    ...
    print(f"Invalid Liquidity Skips in Execution: {len(skipped_liquidity)} ({len(skipped_liquidity)/len(df)*100:.1f}%)")
    
    tdf = pd.DataFrame(trades)
    if len(tdf) > 0:
        wr = len(tdf[tdf["exit_reason"] == "target"]) / len(tdf)
        ev = tdf["return"].mean()
        med = tdf["return"].median()
        sr = len(tdf[tdf["exit_reason"] == "stop"]) / len(tdf)
        
        print(f"Valid Executable Trades: {len(tdf)}")
        print(f"Realized Win Rate: {wr*100:.1f}%")
        print(f"Realized Stop Rate: {sr*100:.1f}%")
        print(f"Realized EV: {ev*100:.2f}% per trade")
        print(f"Median Return: {med*100:.2f}%")
    else:
        print("No valid trades found after liquidity filtering.")
