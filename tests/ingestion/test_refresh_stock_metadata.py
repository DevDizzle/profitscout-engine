...
class TestRefreshStockMetadata(unittest.TestCase):
    ...
    def test_get_existing_metadata_status(self):
        ...
        status = refresh_stock_metadata._get_existing_metadata_status(
            self.mock_bq_client, tickers
        )
        ...
        self.assertIsNone(
            status.get("MSFT")
        )  # Should be None (initialized in function but not in DF)
    ...
    def test_fetch_latest_transcripts_bulk(self):
        ...
        results = refresh_stock_metadata._fetch_latest_transcripts_bulk(
            tickers, self.mock_fmp_client
        )
    ...
    @patch(
        "src.ingestion.core.pipelines.refresh_stock_metadata._fetch_latest_transcripts_bulk"
    )
    @patch(
        "src.ingestion.core.pipelines.refresh_stock_metadata._get_existing_metadata_status"
    )
    def test_run_pipeline_logic(
        self,
        mock_get_status,
        mock_fetch_transcripts,
        mock_fetch_profiles,
        mock_get_tickers,
    ):
        # Setup
