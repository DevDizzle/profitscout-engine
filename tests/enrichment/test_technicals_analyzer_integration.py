...
class TestTechnicalsAnalyzerIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_technicals = {
            "technicals": [
                {"date": "2023-10-25", "RSI_14": 55.0, "SMA_50": 150.0},
                {"date": "2023-10-26", "RSI_14": 60.0, "SMA_50": 151.0}
            ]
        }
        self.sample_prices = {
            "prices": [
                {"date": "2023-10-25", "close": 152.0},
                {"date": "2023-10-26", "close": 155.0}
            ]
        }
    ...
    def test_process_blob_client_initialization_and_execution(self, mock_write, mock_read, mock_genai_client):
        """
        Verifies that:
        ...
        mock_chunk.text = '{"score": 0.85, "strategy_bias": "Bullish", "analysis": "Looks good."}'
        ...
        result = technicals_analyzer.process_blob(blob_name)
        ...
        _, kwargs = call_args
        self.assertEqual(kwargs.get("location"), "global", "Client must be initialized with location='global'")
        ...
        self.assertEqual(gen_config.response_mime_type, "application/json", "Must request JSON output")
