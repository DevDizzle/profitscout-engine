...
def _init_client() -> genai.Client | None:
    ...
    try:
        _log.info("Initializing Vertex AI GenAI client...")
...
@retry(retry=retry_if_exception_type(Exception), wait=wait_exponential(multiplier=2, max=60), stop=stop_after_attempt(5), reraise=True)
def generate(prompt: str) -> str:
    ...
    resp = _client.models.generate_content(model=config.MODEL_NAME, contents=prompt, config=cfg)
