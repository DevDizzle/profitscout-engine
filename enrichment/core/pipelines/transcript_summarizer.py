...
INPUT_PREFIX = config.PREFIXES["transcript_summarizer"]["input"]
OUTPUT_PREFIX = config.PREFIXES["transcript_summarizer"]["output"]
...
def process_blob(blob_name: str):
    ...
    logging.info(f"[{ticker}] Generating transcript summary for {date_str}")
...
def run_pipeline():
    ...
    work_items = [
        ...
        if not f"{OUTPUT_PREFIX}{os.path.basename(blob).replace('.json', '.txt')}" in all_summaries
    ]
    ...
    logging.info(f"--- Transcript Summarizer Pipeline Finished. Processed {count} new files. ---")
