...
# MODIFIED: Input is now the raw transcript from the summarizer's original input folder
...
def read_transcript_content(raw_json: str) -> str | None:
    ...
    try:
        ...
        # Handle cases where the JSON is a list with one element
...
def run_pipeline():
    ...
    for _, row in work_list_df.iterrows():
        ...
        # Check if the final .json analysis file exists
